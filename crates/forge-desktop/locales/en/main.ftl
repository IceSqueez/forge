...
settings_ws_origins_subtitle = One per line. Needed when a browser page loads from an address Forge cannot derive from the bind address - a LAN dashboard or a browser source pointed at your machine's IP. Without a match the WebSocket handshake is rejected.
