...
mod tests {
    ...
    #[tokio::test]
    async fn execute_extracts_parameter_fields_into_arg_stack() {
        let runner = LookupParametersRunner::new(Arc::new(MockSink::new()));
        ...
        assert_eq!(
            out.get("vtube.parameters.names"),
            Some(&Variant::Array(vec![
                Variant::String("FaceAngleX".to_owned()),
                Variant::String("MouthOpen".to_owned()),
            ]))
        );
        assert_eq!(out.get("vtube.parameters.count"), Some(&Variant::Int(2)));
    }

    #[tokio::test]
    async fn execute_on_sink_error_fails_with_no_arg_stack() {
        let runner = LookupParametersRunner::new(Arc::new(MockSink::failing()));
        let stack = ArgStack::new();
        let ctx = make_ctx(&stack);
        let (tel, extra) = runner.execute(&BTreeMap::new(), &ctx).await;
