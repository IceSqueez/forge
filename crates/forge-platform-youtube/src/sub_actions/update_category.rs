...
mod tests {
    ...
    fn runner_on(server: &MockServer) -> UpdateCategoryRunner {
        ...
        UpdateCategoryRunner::new(Arc::new(meta))
    }

    fn config(value: &str) -> SubActionConfig {
        BTreeMap::from([("category_id".to_owned(), Variant::String(value.to_owned()))])
    }

    #[tokio::test]
    async fn execute_interpolates_category_template_then_succeeds() {
        ...
        let stack = ArgStack::new().set("cat".to_owned(), Variant::String("24".to_owned()));

        let (telemetry, _) = runner.execute(&config("%cat%"), &make_ctx(&stack)).await;
    ...
    #[tokio::test]
    async fn empty_category_after_interpolation_fails_without_http() {
        let server = MockServer::start().await;
        let runner = runner_on(&server);
        ...
        assert!(
            ...
            "empty category must not reach the transport"
        );
    }
    ...
    fn validate_config_accepts_valid_rejects_empty_missing_and_non_string() {
        ...
        let runner = UpdateCategoryRunner::new(Arc::new(meta));

        let cases: Vec<(&str, SubActionConfig, bool)> = vec![
            ("valid", config("20"), true),
            ("empty", config(""), false),
            ("missing", BTreeMap::new(), false),
            (
                "non-string",
                BTreeMap::from([("category_id".to_owned(), Variant::Int(20))]),
                false,
            ),
        ];
