...
pub use guest_star_end_session::GuestStarEndSessionRunner;
pub use guest_star_invite::GuestStarInviteRunner;
pub use guest_star_remove_guest::GuestStarRemoveGuestRunner;
pub use guest_star_update_slot::GuestStarUpdateSlotRunner;
...
pub fn register_twitch_sub_actions(
    ...
    reg.register(Box::new(GuestStarUpdateSlotRunner::new(
        Arc::clone(&transport),
        Arc::clone(&identity),
    )))?;
    reg.register(Box::new(GuestStarEndSessionRunner::new(
        transport, identity,
    )))?;
    Ok(())
}
